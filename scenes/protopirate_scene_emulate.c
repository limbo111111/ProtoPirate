        ...
        case ProtoPirateCustomEventEmulateTransmit:
            if (emulate_context && emulate_context->transmitter && emulate_context->flipper_format)
            {
                ...
                if (!flipper_format_read_string(emulate_context->flipper_format, "Preset", preset_str)) {
                    FURI_LOG_W(TAG, "Failed to read preset from file, using AM650 as default");
                ...
                if(!preset_data) {
                    FURI_LOG_W(TAG, "Preset '%s' not found, falling back to AM650", preset_name);
                    furi_string_set_str(preset_str, "AM650");
                    preset_name = furi_string_get_cstr(preset_str);
                ...
                if(preset_data) {
                    ...
                    FURI_LOG_E(TAG, "All fallback presets failed. Cannot transmit.");
                    notification_message(app->notifications, &sequence_error);
                }
